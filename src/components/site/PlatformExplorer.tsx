...
export function PlatformExplorer() {
  ...
  return (
    <div
      className="rounded-2xl border border-border bg-(--ink) overflow-hidden shadow-(--shadow-glow)"
    ...
    >
      <div className="flex items-center gap-2 px-4 py-3 border-b border-white/8 bg-black/30">
        ...
        <div className="w-7 h-7 rounded-full bg-linear-to-br from-accent to-primary shrink-0" />
      </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-5">
            ...
            <div className="flex gap-2 shrink-0 flex-wrap">
              {mod.metrics.map((m) => (
                <div
                  key={m.label}
                  className="rounded-lg border border-white/8 bg-white/4 px-3 py-2 text-center min-w-18"
                >
                ...
                <div className="flex items-end gap-0.5 h-8">
                  {Array.from({ length: 8 }, (_, j) => 30 + Math.sin(j + i) * 20 + h * 0.3).map(
                    (bar, j) => (
                      <div
                        key={j}
                        className="flex-1 rounded-sm bg-linear-to-t from-primary/60 to-accent/40"
                        style={{ height: `${bar}%` }}
                      />
                    ),
                  )}
...
export function LiveActivityFeed() {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card overflow-hidden shadow-(--shadow-soft) h-full flex flex-col">
...
export function EnterpriseMatrix() {
  return (
    <div className="rounded-2xl border border-border overflow-hidden shadow-(--shadow-soft) h-full">
...
export function CodePreview() {
  return (
    <div className="rounded-2xl border border-border overflow-hidden shadow-(--shadow-glow) bg-(--ink)">
